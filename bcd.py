...
csv_data = pd.read_csv(r"E:\6th Sem\Soft Computing\PROJECT\data.csv")
...
sm = scatter_matrix(csv_data[featuresMean], c=colors, alpha=0.4, figsize=((10, 10)))
plt.show()
...
for i, feature in enumerate(featuresMean):
    ...
    sns.displot(csv_data[csv_data['diagnosis'] == 'M'][feature], bins=bins, color='red', label='M')
    sns.displot(csv_data[csv_data['diagnosis'] == 'B'][feature], bins=bins, color='blue', label='B')
